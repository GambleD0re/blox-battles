    ...
    return (
        <div className="flex-shrink-0 w-full h-20 bg-gray-900/60 border border-gray-700 rounded-lg p-2 flex items-center justify-between gap-2">
            <div className="flex items-center gap-3 flex-1 min-w-0">
                <img src={winner.avatarUrl} alt={winner.username} className="w-14 h-14 object-cover rounded-full border-2 border-green-400" />
            ...
            <div className="text-center flex-shrink-0 mx-4">
                ...
                <div className="font-bold text-sm text-green-400">{pot.toLocaleString()} Gems</div>
            </div>
            <div className="flex items-center justify-end gap-3 flex-1 min-w-0">
                <span className="font-bold text-white text-lg truncate text-right">{loser.username}</span>
                <img src={loser.avatarUrl} alt={loser.username} className="w-14 h-14 object-cover rounded-full border-2 border-gray-600" />
            </div>
        </div>
    );
};
...
const LiveFeed = ({ token, onMatchFound, onInboxRefresh }) => {
        ...
        const connect = () => {
            ...
            ws.current.onmessage = (event) => {
                ...
                if (data.type === 'live_feed_history') {
                    setDuels(data.payload.map((duelData, i) => ({ key: `hist-${duelData.id}`, pos: i === 0 ? 'slot1' : 'slot2', data: duelData })));
                } else if (data.type === 'live_feed_update') {
                    setDuels(current => [{ key: `new-${data.payload.id}`, pos: 'enter', data: data.payload }, ...current.map(d => ({...d, pos: d.pos === 'slot1' ? 'slot2' : 'exit'}))]);
    ...
    useEffect(() => {
        const enterTimer = setTimeout(() => setDuels(current => current.map(d => d.pos === 'enter' ? {...d, pos: 'slot1'} : d)), 100);
        const exitTimer = setTimeout(() => setDuels(current => current.filter(d => d.pos !== 'exit')), 800);
    ...
    return (
        <div className={`fixed bottom-0 left-0 right-0 z-40 transition-transform duration-300 ${isVisible ? 'translate-y-0' : 'translate-y-full'}`}>
            ...
            <div className="h-28 bg-black/60 backdrop-blur-md border-t-2 border-gray-800 flex items-center overflow-hidden">
                ...
                <div className="flex-grow h-full"><div className="live-feed-cards-container">{duels.map(d => <div key={d.key} className={`duel-card-wrapper pos-${d.pos}`}><DuelCard duel={d.data} /></div>)}</div></div>
