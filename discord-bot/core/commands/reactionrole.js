// discord-bot/core/commands/reactionrole.js
...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(subcommand =>
            subcommand.setName('setup').setDescription('Creates a new message for reaction roles.')
        ...
        .addSubcommand(subcommand =>
            subcommand.setName('add').setDescription('Adds a role-to-emoji mapping to a message.')
        ...
        .addSubcommand(subcommand =>
            subcommand.setName('remove').setDescription('Removes a role-to-emoji mapping from a message.')
        ...
        .addSubcommand(subcommand =>
            subcommand.setName('list').setDescription('Lists all configured reaction roles for a message.')
    ...
    async execute(interaction) {
        ...
        try {
            if (subcommand === 'setup') {
                ...
                const emojiId = emoji.match(/\d{18}/)?.[0] || emoji;
                ...
                if (targetMessage) await targetMessage.react(emoji);
                await interaction.editReply(`✅ Rule added: ${emoji} will now grant the **${role.name}** role.`);
                ...
                if (rules.length === 0) return interaction.editReply('No reaction roles found for that message.');
            ...
            await interaction.editReply(`❌ **Error:** ${error.response?.data?.message || 'An error occurred.'}`);
        }
    },
};
