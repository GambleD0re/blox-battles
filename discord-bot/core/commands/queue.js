...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('queue')
        .setDescription('Join or leave the matchmaking queue for a specific game.')
        .addSubcommand(subcommand =>
            subcommand
                .setName('join')
                .setDescription('Join the matchmaking queue for a game.')
        ...
        .addSubcommand(subcommand =>
            subcommand
                .setName('leave')
                .setDescription('Leave the matchmaking queue for a game.')
                .addStringOption(option =>
                    option.setName('game')
                        .setDescription('The game queue you want to leave.')
                        .setRequired(true)
                        .addChoices(...getGameChoices()))),
    async execute(interaction) {
        ...
        if (subcommand === 'join') {
            ...
            await interaction.deferReply({ ephemeral: true });
            try {
                await apiClient.post(`/games/${gameId}/queue/leave`);
                await interaction.editReply({ content: `You have successfully left the ${gameName} queue.` });
            } catch (error) {
                await interaction.editReply({ content: `❌ ${error.response?.data?.message || 'An error occurred.'}` });
            }
        }
