...
--[[
    ...
    Version: 27.0.0 (Multi-Game Compatibility)
    Date: 2025-08-22
    
    Features:
    - [NEW] Multi-game awareness via GAME_ID.
    - [NEW] Updated API endpoints for multi-game backend.
    ...
    - Anti-AFK (Autokick Protection).
    - Self-termination via client freeze upon being kicked.
]]
...
local GAME_ID = "rivals" -- [MODIFIED] Added Game ID for multi-game backend
...
if loadControllersAndModules() then
    ...
    LocalPlayer.AncestryChanged:Connect(function(_, parent) if parent == nil then warn("Bot: KICKED or DISCONNECTED. Terminating instance by freezing."); while true do task.wait(1) end end end)
    ...
    task.spawn(function() while true do task.wait(ANTI_AFK_INTERVAL); pcall(function() VirtualUser:Button1Down(Vector2.new(0, 0)); task.wait(0.1); VirtualUser:Button1Up(Vector2.new(0, 0)); print("Bot: Anti-AFK simulated input.") end) end end)
else
