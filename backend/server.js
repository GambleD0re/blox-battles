...
This second script updates the `/api/config` endpoint. It now tells the frontend whether the blockchain features are enabled, allowing the UI to adapt accordingly.
...
router.get('/', (req, res) => {
    try {
        ...
        const config = {
            ...
            // [ADDED] Expose the blockchain feature status to the frontend
            blockchainFeaturesEnabled: process.env.BLOCKCHAIN_FEATURES_ENABLED !== 'false'
        };
